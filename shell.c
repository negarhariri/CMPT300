...
int main(int argc, char **argv)
{
	...
	int counter = 1;
	int sub_counter = 0 ;
	int new_pid_from_fork = 0 ;
	char HistoryBuffer[SHELL_BUFFER_SIZE];
...
if(buffer[0]== '!')
{
	int cmdBefor = buffer[1]-'0';
	if(1<= cmdBefor&& cmdBefor <= 9  &&(cmdBefor <counter))
	{
		strncpy(buffer , &HistoryBuffer[cmdBefor], SHELL_BUFFER_SIZE);
	}
...
if(buffer[0] != '\n')
{
	...
	if(counter <= 9 )
	{
		strncpy(&HistoryBuffer[counter] , buffer ,SHELL_BUFFER_SIZE);
...
buffer[i] = '\0';
/* Terminate input, overwriting the '&' if it exists */
    ...
    {   sub_counter++;
            ...
            
		{
				...
				if (!strcmp(exec_argv[0],"sub"))
				{
					...
					imthechild(exec_argv[0], &exec_argv[0]);
